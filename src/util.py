...
def save_taskC(
        ...
        **kwargs) -> bool:
    assert output is not None or df is not None, "Both output and df are none!"
    if output is None:
        output = df
    assert "note" in output.columns, "`note' column not found in the given dataframe to save output!"
    _divisions = ["subjective", "objective_exam", "objective_results", "assessment_and_plan"]
    if any([i not in output.columns for i in _divisions]):
        output = output.apply(lambda row: add_section_divisions(row, "note"), axis=1)
    ...
    output = output.apply(lambda row: convert_to_str(row), axis=1)
    output.to_csv(output_file_path, index=False,
                  columns=["encounter_id", "note", "subjective", "objective_exam", "objective_results",
                           "assessment_and_plan", ])
    return True
