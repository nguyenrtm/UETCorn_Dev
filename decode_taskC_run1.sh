...
if [ ! -f "cache/$prefix-cache-v6.csv" ]; then
  # Execute the Python script

  # run preprocessing code
  python main.py -p "preprocessing" \
      -i $1 \
      -o "cache/$prefix-cache-v6.csv" \
      -dc "dialogue" \
      -ic "encounter_id" \
      -pc "config/bart-retrieval-v6-nogroup.json"
...
python main.py -i "cache/$prefix-cache-v6.csv"  \
  -o "run_results/$prefix-taskC_uetcorn_run1_raw.csv" \
  -dc "retrieval_text" \
  -ic "encounter_id" \
  -p "note-complete" \
  -pc "config/bart-retrieval-v6-nogroup.json"
...
python main.py -i "run_results/$prefix-taskC_uetcorn_run1_raw.csv" \
  -o "taskC_uetcorn_run1_mediqaSum.csv" \
  -dc "note" \
  -ic "encounter_id" \
  -p "postprocessing" \
  -pc "config/bart-retrieval-v6-nogroup.json"
