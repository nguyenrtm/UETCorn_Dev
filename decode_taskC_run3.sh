...
if [ ! -f "cache/$prefix-cache-v7.csv" ]; then
  # Execute the Python script

  # run preprocessing code
  python main.py -p "preprocessing" \
      -i $1 \
      -o "cache/$prefix-cache-v7.csv" \
      -dc "dialogue" \
      -ic "encounter_id" \
      -pc "config/bart-retrieval-v7.json"
...
python main.py -i "cache/$prefix-cache-v7.csv"  \
  -o "run_results/$prefix-taskC_uetcorn_run3_raw.csv" \
  -dc "retrieval_text" \
  -ic "encounter_id" \
  -p "note-complete" \
  -pc "config/bart-retrieval-v7.json"
...
python main.py -i "run_results/$prefix-taskC_uetcorn_run3_raw.csv" \
  -o "taskC_uetcorn_run3_mediqaSum.csv" \
  -dc "note" \
  -ic "encounter_id" \
  -p "postprocessing" \
  -pc "config/bart-retrieval-v7.json"
